...
// parametros reclutamientos y mortalidades)
 ...
 init_bounded_vector log_Fm(1,ntime,-20,5,opt_F) // log  mortalidad por pesca por flota
 init_bounded_vector log_Fh(1,ntime,-20,5,opt_F) // log  mortalidad por pesca por flota
...
// vector BMflo(1,ntime)
 ...
 sdreport_vector Flasth(1,ntime)//
 sdreport_vector Flastm(1,ntime)//
...
FUNCTION tac_proj
 ...
 for (j=1; j<=npbr+1; j++){
   ...
   for (i=1; i<=ntime_sim; i++){
     ...
     Zpbrm = Fpbrm + Mm;
     Spm   = exp(-1.*Zpbrm);
     tac_m(i,j) = sum(elem_prod(elem_prod(elem_div(Fpbrm,Zpbrm),elem_prod(Npm,(1-Spm)))*Prob_talla_m,Wmed(1)));
     ssb_m(i,j) = sum(elem_prod(elem_prod(elem_prod(Npm,mfexp(-dt(1)*Zpbrm))*Prob_talla_m,msex),Wmed(1)));
